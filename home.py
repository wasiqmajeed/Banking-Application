...
if customer_info == 1:
    ...
    if find_user(user_account): # Need to later change it to if user_account in users and user_password == passwords, currently just checking with users
        ...
        current_or_saving = int(input('Please press 1 for Checking account, Press 2 for Savings account.\n'))

        if user_choice == 1:

            #User can check balance in current and saving account
            if current_or_saving == 1:
                print(CheckingAccount.total_balance(user_account))
            ...
            if current_or_saving  == 1:
                CheckingAccount.debit_money(user_account, debit_amount)
            else:
                print(SavingAccount.debit_money(user_account, debit_amount))
            ...
            if current_or_saving == 1:
                print(CheckingAccount.credit_money(user_account, amount)  )
            else:
                print(SavingAccount.credit_money(user_account, amount))
